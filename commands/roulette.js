...
module.exports = {
    name: 'roulette',
    description: `Test your luck to see if you win or lose Levels!\nUse the \`--risk\` flag to include the more risky results that can affect multiple Users!`,
    usage: '<bet> [--risk]',
    aliases: ['roul', 'r'],
    args: true,
    guildOnly: true,
    ...
    async execute(message, args) {
      ...
      // Check if "--risk" flag was given
      if (message.content.includes("--risk")) {
        ...
        roulEmbed.setTitle(`${message.member.displayName} spun the Risky Roulette!`);
        let lvlValue;
        let tenPercent;
        let fiftyPercent;
        let hundredPercent;
        ...
        // No flag was given, use standard Roulette
        let result;
        result = chance.weighted(
          ['nothing', 'lose', 'win10', 'win50', 'win100', 'win200', 'winlevel', 'win3levels', 'lose10', 'lose50', 'lose100', 'lose200', 'loselevel', 'lose3levels'],
